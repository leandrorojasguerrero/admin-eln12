  ...
  public function notifUserPostAction(Request $request)
    ...
    public function api_latestAction(Request $request,$language,$token){
        ...
        if ($language_obj == null) {
            $categories_bjs =   $em->getRepository("AppBundle:Category")->findBy(array(),array("position"=>"asc"));
        }else{
        ...
        if ($language != 0) {
            ...
            $questions_objs =  $em->getRepository("AppBundle:Question")->findBy(array("enabled"=>true,"featured"=>true));
        }
